        ...
        <form action="{{ route('equipos.store') }}" method="POST">
            ...
            <div class="mt-4">
                ...
                <select name="id_usuario" id="id_usuario" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus-within:text-primary-600 focus:border-primary-300 focus:ring-primary-200 focus:ring focus:ring-opacity-50">
                    ...
                    @foreach($usuarios as $usuario)
                        <option value="{{ $usuario->id }}" @selected(old('assigned_to', []))>{{ $usuario->name }}</option>
                    @endforeach 
                </select>
            ...
            <div class="mt-4">
                ...
                <select id="tipo" name="tipo" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-300 focus:ring-primary-200 focus:ring focus:ring-opacity-50" required>
                    ...
                    <option value="Escáner" {{ old('tipo') == 'Escáner' ? 'selected' : '' }}>Escáner</option>
                    ...
                    <option value="Cableado" {{ old('tipo') == 'Cableado' ? 'selected' : '' }}>Cableado</option>
                    <option value="Cocina" {{ old('tipo') == 'Cocina' ? 'selected' : '' }}>Cocina</option>
                    <option value="Baños" {{ old('tipo') == 'Baños' ? 'selected' : '' }}>Baños</option>
                </select>
            ...
            <input type="hidden" name="direccion_ip" value="{{ $ipAddress }}">       
