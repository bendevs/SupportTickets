...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const errorDiv = document.querySelector('.mb-4.text-sm.font-medium.text-red-600');
        if (errorDiv) {
            setTimeout(() => {
    ...
    <div class="rounded-lg bg-white p-4 shadow-md">
        ...
        <form action="{{ route('tickets.store') }}" method="POST" enctype="multipart/form-data">
            ...
            @if (auth()->user()->hasRole('admin') || auth()->user()->hasRole('agent'))
                <div class="mt-4">
                    <x-input-label for="assigned_to_user" :value="__('Asignación de Usuario')" />
                    <select name="assigned_to_user" id="assigned_to_user" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus-within:text-primary-600 focus:border-primary-300 focus:ring-primary-200 focus:ring focus:ring-opacity-50">
                        <option value="" selected disabled>-- SELECCIONAR USUARIO DE EQUIPO --</option>
                        @foreach($allUsers as $user)
                            <option value="{{ $user->id }}" @selected(old('assigned_to_user') == $user->id)>{{ $user->name }}</option>
                        @endforeach 
                    </select>
                    <x-input-error :messages="$errors->get('assigned_to_user')" class="mt-2" />
                </div>
            @else
            ...
            <div class="mt-4">
                ...
                <select class="block w-full rounded-md border-gray-300 shadow-sm focus-within:text-primary-600 focus:border-primary-300 focus:ring-primary-200 focus:ring focus:ring-opacity-50" name="equipo" id="equipo" required>
                    <option value="" selected disabled>-- SELECCIONAR EQUIPO --</option>
                    @foreach($equipos as $equipo)
                        <option value="{{ $equipo->id }}">{{ $equipo->nombre_equipo }} - @foreach ($allUsers as $users)
                            @if ($users->id == $equipo->id_usuario)
                                {{ $users->name }}
                            @endif
                        @endforeach</option>
                    @endforeach
            ...
            {{-- <div class="mt-4">
                <x-input-label for="assigned_to" :value="__('Departamento')" />
                <select class="block w-full rounded-md border-gray-300 shadow-sm focus-within:text-primary-600 focus:border-primary-300 focus:ring-primary-200 focus:ring focus:ring-opacity-50" name="departamento" id="departamento">
