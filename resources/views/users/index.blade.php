                        ...
                        @foreach($users as $user)
                            <tr class="text-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <span class="rounded-full bg-gray-50 px-2 py-2">
                                        @if ($user->roles()->first()->name === "admin")
                                            Administrador
                                        @elseif ($user->roles()->first()->name === "agent")
                                            Agente
                                ...
                                <td class="px-4 py-3 space-x-2">
                                    ...
                                    <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('Esta usted seguro en eliminar?')" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <x-primary-button>
                                            Eliminar
