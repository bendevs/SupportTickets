<x-app-layout>
    <x-slot name="header">
        {{ __('Actualizar especialidad del personal') }}
    </x-slot>

    <div class="rounded-lg bg-white p-4 shadow-md">

        <form action="{{ route('personalEspecialidad.update', $personal) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mt-4">
                    ...
                    @foreach($usuarios as $usuario)
                        @if ($usuario->roles()->first()->name === "admin" || $usuario->roles()->first()->name === "agent")
                            <option value="{{ $usuario->id }}" 
                                @if(old('id_users') == $usuario->id || $usuario->id == $personal->id_users) 
                                    selected 
                                @endif>
                                {{ $usuario->name }}
                            </option>
                ...
                <select name="id_especialidades" id="id_especialidades" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus-within:text-primary-600 focus:border-primary-300 focus:ring-primary-200 focus:ring focus:ring-opacity-50">
                    ...
                    @foreach($tecnicos as $tecnico)
                        <option value="{{ $tecnico->id }}" 
                            @if(old('id_especialidades') == $tecnico->id || $tecnico->id == $personal->id_especialidades) 
                                selected 
                            @endif>
                        {{ $tecnico->especialidad }}</option>    
                    @endforeach 
                </select>
